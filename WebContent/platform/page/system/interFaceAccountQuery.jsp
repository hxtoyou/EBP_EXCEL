...
var txnNo = '<%=txnNo%>';
	$(function() {
		...
		$("#interfaceInfo").datagrid({
			rownumbers : true,
			url : $xcp.def.getFullUrl('./interface.do?method=queryAccoutnIntfacefInfo&txnNo='+$("#txnNo").xcpVal()),
			pagination : true,
			fitColumns : true,
			singleSelect: true,
			width : "900",
		...
		var strBtn = "<button type=\"button\" class=\"paramShowMoreBtn\" id=\"intfHisQuery\" style=\"margin-left: 10px; width:120px\" id=\queryIntf\">"+
			$xcp.i18n("itstu.intfHistoryQuery")+"</button>";
		$(strBtn).appendTo($("#northDiv"));
		
		var strBtn2 = "<button type=\"button\" class=\"paramShowMoreBtn\" id=\"intfHisQuery2\" style=\"margin-left:10px; width:140px\" id=\queryIntf\">"+
		$xcp.i18n("itstuInterface.checkAcctList")+"</button>";
		$(strBtn2).appendTo($("#northDiv"));
		...
		$("#intfHisQuery2").unbind("click.intf").bind("click.intf",function(){			
			checkAcctList();
		});
	});
	
	function checkAcctList(){
		var txnNo=$("#txnNo").xcpVal();
		var userId="EBP01";
		var tranOrgNo="5901";
		var data = $xcp.intfQuery("ITSASE",{'torfno':txnNo,'tellerno':userId,'brno':tranOrgNo});
		var returnDate = $xcp.parseJson(data);
		var sucflg = returnDate.sucflg;
		var errorCode = returnDate.errorCode;
		var errorMsg = returnDate.errorMsg;
		if(errorCode=='00000000'){
			if(sucflg=='C'){
				$.messager.alert($xcp.i18n("sys.tip"),$xcp.i18n('itstuInterface.checkSuccess'),'warning');
			}else{
				$.messager.alert($xcp.i18n("sys.tip"),$xcp.i18n('itstuInterface.checkFail'),'warning');
			}
		}else{
			$.messager.alert($xcp.i18n("sys.tip"),$xcp.i18n('itstuInterface.return')+errorMsg,'warning');
		}
		flag = "Y";//20150105 东莞新增，在head.jsp中新增一个全局变量flag，用来校验在记账人工确认中是否检查了核心是否记账成功
	}
	...
	function intfHisQuery(){
		 ...
		 $("#infoList").datagrid({
				...
				singleSelect: true,
				url:$xcp.def.getFullUrl('./interface.do?method=queryTradeIntfInfo&txnNo='+$("#txnNo").xcpVal()),
				width: "400",
				height: "auto",
				fit   : true,
				fitColumns: true,
