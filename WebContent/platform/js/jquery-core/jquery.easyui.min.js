...
(function($) {
	$.parser = {
		...
		plugins : [ 
				"combobox","linkbutton", 
				"numberbox", "validatebox", "searchbox",
				"datebox", "menu","datagrid",
				"layout", "panel","tree","combotree","menu",
		 ...
		 */				
		parse : function(_2,tabsGroup, bfplugins) {
			var plugins =  bfplugins || $.parser.plugins;
			
			if(tabsGroup === true && (_2 == null || _2 == undefined)){
				plugins = [ "layout","tabs" ];
			}
			...
			for ( var i = 0; i < plugins.length; i++) {
				...
				//lishuai 添加初始化combobox 验证问题
				if(plugins[i] == "combobox"){
					$(".validatebox-invalid",_2).removeClass("validatebox-invalid");
				}
			}
	...
	$(function() {
		...
		if (!window.easyloader && $.parser.auto) {	
			 ...
			 $.parser.parse(null,window.easyui_parse_process_group === true);
		}
	});
...
(function($) {
	...
	function _27(e) {
		...
		if (_2a.revert) {
			if (_2b() == true) {
				if(e.data.startPosition == 'relative'){
					$(e.data.target).css({
		...
		return this.each(function() {
			...
			if (_170) {
				...
				if (data.length && !opts.data) {
					opts.data = data;
				}else if(!opts.data && $.isFunction(opts.loadDataFun)){//ls 20150410
					opts.data = opts.loadDataFun(this);
				}
			}
			_b5(this);
			if (opts.lines) {
...
(function($) {
	...
	;
	function _1e5(_1e6, _1e7,isInit) {
...
(function($) {
	...
	function _22d(_22e) {
		...
		if (_22f.options.shadow == true) {
			_22f.shadow = $("<div class=\"window-shadow\"><iframe style='width:100%; heigth:100%; z-index: 9999; border: none;' src='about:blank'></iframe></div>").insertAfter(
					_22f.window);
			_22f.shadow.css({
				display : "none"
			});
...
(function($) {
	...
	$.messager = {
		...
		alert : function(_264, msg, icon, fn) {
			...
			//added by tx messageer alert 点击关闭按钮时不调用回调函数的处理
			win.window({
				onBeforeClose:function(){
					if(fn){
						fn();
			...
			//gy append by 2013-11-11
			win.window("window").css({"zIndex":100000});
			return win;
		},
		...
		progress : function(_26d) {
			var _26e = {
				...
				close : function() {
					...
					if (win.length) {
						$('div.window-mask').css({"zIndex":9007});
						win.window("close");
					}
				}
			};
			...
			win.window({
				closable : true,
				closed   : opts.closed === true ? true : false,
				onClose : function() {
					if (this.timer) {
						clearInterval(this.timer);
					}
					if(opts.notNeedDestroy !== true){
						$(this).window("destroy");
					}
				},
				onOpen : function(){
					//gy
					win.window("window").css({"zIndex":10000});
					$('div.window-mask').css({"zIndex":9999});
				}
			});
			if (opts.interval) {
...
(function($) {
	...
	function _2d9(_2da, pp, _2db) {
		...
		pp.panel($.extend({}, _2db, {
			...
			closed : true,
			doSize : false,
			isClicked : false,/*modified by tx 2013-11-27*/
	...
	function _2e1(_2fb, _2fc) {
		...
		if (_2ff) {
			...
			_32a(_2fb, _2dd(_2fb, _2ff),_2fe);
			if (!_2ff.panel("options").closed) {
				return;
			}
			//增加tab叶签离开点击事件 onUnselect 
			/*_2ff.panel("close");  
		...
		/*modified by tx 2013-11-27*/
		_2fe.panel("options").isClicked = true;
		
	}
	;
	function _32a(_32e, _32f,_2fe) {
		var _330 = $.data(_32e, "tabs");
		var p = _2f0(_32e, _32f);
		if (p) {
			var opts = p.panel("options");
			if (!opts.closed) {
				var _0728 = _330.options.onUnselect.call(_32e, opts.title,
						_2dd(_32e, p),_2fe);
				if(_0728 == false)return false;
				p.panel("close");
				if (opts.closed) {
	...
	$.fn.tabs.parseOptions = function(_312) {
		return $.extend({}, $.parser.parseOptions(_312, [ "width", "height",
				"tools", "toolPosition", "tabPosition", {
					...
					plain : "boolean",
					headerWidth : "number",
					isClicked : "boolean"  /*modified by tx 2013-11-27*/
				} ]));
	};
	$.fn.tabs.defaults = {
		width : "auto",
...
(function($) {
	function init(_3ad,opts) {
		var span = null;
		if($(_3ad).attr('htmlStaticed') == 'Y'){		
			 span = $(_3ad).next();
		}else{
			$(_3ad).hide();		
			var name = $(_3ad).attr("name");					
			if (name) {
				//有name属性的是业务表单字段,作特殊处理
				span = $("<span class=\"searchbox\" style=\"width: 200px; height: 20px;\"><input type=\"text\" name=\"" + name + "\" style=\"width: 182px; height: 25px; line-height: 25px;\" class=\"searchbox-text\"><span><span class=\"searchbox-button\" style=\"height: 25px;\" ></span></span></span>").insertAfter(_3ad);;
				//_3ae.attr("name", name);
				$(_3ad).removeAttr("name").attr("searchboxName", name).attr("componentName" , 'searchbox');
			}else{
				var width = $(_3ad).width();
				span = $("<span class=\"searchbox\" style='width :" + width +"px';height:22px;><input type=\"text\"  class=\"searchbox-text\" style='width:" + (width - 18)+ "px;height:20px; line-height: 22px;'><span><span class=\"searchbox-button\" style=\"height: 22px;\"></span></span></span>").insertAfter(_3ad);;
				$(_3ad).attr("componentName" , 'searchbox');
			}		
		}	
		return span;
	}
	...
	function _3ba(_3bb) {
		...
		//增加validatebox 校验 modified by tx
		if ($.fn.validatebox) {
			$(_3bd).validatebox(opts);
		}
		
		var cls =$(_3bb).attr('class');
		if(cls != ''){
			 $(_3bd).addClass(cls.replace('easyui-searchbox',''));
		}
		...
		_3bd.unbind(".searchbox").bind("blur.searchbox", function(e) {
			...
			if (e.keyCode == 13) {
				...
				$(_3bb).trigger("keydown.srchBox",[e]);
				return false;
			}
		}).bind("change.searchbox", function() {  //modified 
			  $(_3bb).xcpVal($(this).val()); //lsm 触发searchbox onchange事件
			  $(_3bb).trigger("change");
		});
		_3be.unbind(".searchbox").bind("click.searchbox", function() {
//			if($(_3bb).hasClass("P"))return;//禁用searchbox  因为我们平台用的自定义事件所以屏蔽
			var name = $.fn.prop ? _3bd.prop("name") : _3bd.attr("name");
			opts.searcher.call(_3bb, opts.value, name);
			
			 $(_3bb).trigger("click",_3bb); //modified
	...
	$.fn.searchbox = function(_3c3, _3c4) {
		...
		return this.each(function() {
			...
			if (_3c5) {
				...
				var opts =  $.extend({componentName:'searchbox'}, $.fn.searchbox.defaults,$.fn.searchbox.parseOptions(this), _3c3)
				_3c5 = $.data(this, "searchbox", {
					options : opts,
					searchbox : init(this,opts)
				});
				if($(this).hasClass("P")){//初始化P 样式
					 $(this).searchbox("textbox").attr("readonly","readonly");
				};
			}
			_3b5(this);
			_3bf(this);
...
(function($) {
	...
	function _3da(_3db) {
		...
		var box = $(_3db);
		var _3dd = box.val();
		var tagName = $(_3db).xcpGetName($(_3db));
		function _3de(msg) {
			_3dc.message = msg;
		}
		;
		...
		/**如果有下拉框则引用另外一个样式 ls*/
		function addClassValidIfOverflow(_obj){
			$(_obj).addClass("validatebox-invalid");
			if($(_obj).hasScrollHeight() || $(_obj).css("overflow-y") == "scroll"){
				$(_obj).css({
					 "background-position":(100 - 18/$(_obj).outerWidth()*100) + "% center"
				});
	    	}
		}
		...
		function _3df(_3e0) {
			...
			var rule = _3e1  && _3e1[1] ? opts.rules[_3e1[1]] : null;
			if (rule && _3dd) {
				var _3e2 = eval(_3e1[2]);
				if (!rule["validator"](_3dd, _3e2,_3db)) { // add _3db modified by tx
					addClassValidIfOverflow(box);
					if(box.hasClass('combo-text')){
						box.parent().addClass('combo-invalid');
					}
					...
					if($xcp.formPubMgr)$xcp.formPubMgr.validatEasyuiError(_3db,tagName+"-"+_3e3);
					return false;
				}
			}
			return true;
		...
		var bolean = true , packBolen = true;
		var MTMopV = $("#" + tagName);
		if(MTMopV && MTMopV.length > 0){
			if($xcp.formPubMgr){
				//做报文和申报的特殊验证处理
				bolean = packBolen = $xcp.formPubMgr.easyuiValid(MTMopV);
			}
			
			if(MTMopV.hasClass("M")){//lsm  做样式显示
				box.addClass("M");
				if(MTMopV.hasClass("optional")){
					box.addClass("optional");
				}
			}else if(MTMopV.hasClass("P")){
				box.addClass("P");
			}else if(MTMopV.hasClass("O")){
				box.addClass("O");
			}
			if(opts.required)
				bolean = bolean &&  (MTMopV.hasClass("M") || !MTMopV.hasClass("O")) ;
			else
				bolean = bolean && MTMopV.hasClass("M");
		}else{
			if($(_3db).parents("div.datagrid-cell").length > 0 && opts.required){
				bolean = true;
			}else{
				bolean = false;
			}
		...
		if (opts.required) {
		    ...
		    if (_3dd == "" &&  bolean) {
				...
				if($xcp.formPubMgr)$xcp.formPubMgr.validatEasyuiError(_3db,tagName+"-"+opts.missingMessage);
				return false;
			}
		}else{  //lsm  else 里面的内容
			if(_3dd == "" &&  bolean){
		...
		if (opts.validType) {
			if (typeof opts.validType == "string") {
				if (packBolen && !_3df(opts.validType)) {
					return false;
				}
			} else {
				for ( var i = 0; i < opts.validType.length; i++) {
					if (packBolen && !_3df(opts.validType[i])) {
						return false;
					}
				}
			}
		...
		if(box.hasClass('combo-text')){
			box.parent().removeClass('combo-invalid');
		}
		_3d1(_3db);
		return true;
	}
	...
	function load(_3f7, data) {
		...
		function _3f9(data) {
			...
			for ( var name in data) {
				var val = data[name]; 	var src = $("#" + name);
				if(src && src.length > 0){
					src.xcpVal(val);
				}else{
					lsm = 0;					
					...
					if (!rr.length) {
						...
						if (f.length) {
							...
							if($("input[name=\"" 
								...
								$("input[searchboxname=\"" + name + "\"]", form).val(val);
								lsm++;
							}
						}
					}else{
					...
					if(lsm == 0){
						$("<input/>").attr("name",name).attr("id",name).attr("value",val).hide().appendTo(form);
					}
				}
			}
			opts.onLoadSuccess.call(_3f7, data);
			//_3fe(_3f7);去掉加载时的validatebox校验  tx
		}
		;
		function _3fa(name, val) {
			...
			rr.each(function() {
				var f = $(this);
				//修改 rodio中val不是数组的问题 if (f.val() == String(val) || $.inArray(f.val(), $.isArray(val)) >= 0) {
...
(function($) {
	function init(_40b) {
		...
		$(_40b).attr("componentName","numberbox");
		if (name) {
			v.attr("name", name);
	...
	function _40e(_411, _412) {
		...
		if (_414 != _412) {
			...
			//$(_414).trigger('change'); //modified  
		}
	}
	;
	function _40eNotEvent(_411, _412) {
		var _413 = $.data(_411, "numberbox");
		var opts = _413.options;
		//lsm6 
		_412 = opts.parser.call(_411, _412);
	...
	function _415(_416) {
		// ls 为了解决jquery change 绑定事件先后顺序问题
		if($(_416).attr("mynumberboxSJeasyui") != undefined ){
			return "";
		}else{
			$(_416).attr("mynumberboxSJeasyui","true");
		}
		...
		$(_416).unbind(".numberbox").bind("keypress.numberbox", function(e) {
			...
			var precision = 2 ;
			if($.isFunction(opts.precision)){
				precision = opts.precision(this);
			}else if($.isNumeric(opts.precision)){
				precision = opts.precision;
			}else if(typeof opts.precision == 'string'){
				precision = $xcp.def ? $xcp.def.getAmtFmtUtil($('#' + opts.precision).xcpVal()) : "";
			}
			
			if((e.which >= 48 && e.which <= 57&& e.ctrlKey == false && e.shiftKey == false)
		    		 || e.which == 45 || e.which == 46){
		    	  if($(this).val().length > 0){
		    	  		var rng = precision == 0 ?  new RegExp("^-?\\d*$") : new RegExp("^-?\\d*\\.?\\d{0," + precision + "}$");
		    	  	    var pos = $xcp.getCurPosition(this);
		    	  	    //modify by xhb 小数位字段不能直接修改的问题的解决
		    	  	    var selectText = $xcp.getSelectedText(this);
		    	  	    var v = $(this).val().substr(0,pos) +  String.fromCharCode(e.which) + $(this).val().substr(pos+selectText.length);
		    	  	    //var v = $(this).val().substr(0,pos) +  String.fromCharCode(e.which) + $(this).val().substr(pos);
		    	 	    return rng.test(v);
		    	  }
		    	  return true;
		     }else{
		...
		}).bind("change.numberbox", function() {
			_40e(_416, $(this).val());
		}).bind("blur.numberbox", function() {
			//_40e(_416, $(this).val()); modified
	...
	function _417(_418) {
		if ($.fn.validatebox) {
			...
			if($.isFunction(opts.precision)){
				...
				precision = $xcp.def.getAmtFmtUtil($('#' + opts.precision).xcpVal());
			}
			
			//报文栏位总长度为14位
			var zNm = $(_418).attr("format") ? $xcp.def.maxFmtUtil - 1 : $xcp.def.maxFmtUtil;
			//判断是否存在validType
			if(!validType){
				validType = 'numberFmt[' + zNm + ',' + precision +']';
			}else if($.isArray[validType] || typeof validType == "object"){	
				//验证是否手动添加了numberFmt校验，已经添加过，就不在默认给其添加
				var flag = true;
				for(var i = validType.length - 1; i >= 0; i--){
					if(validType[i].indexOf('numberFmt') > -1){
						flag = false;
						break;
					}
				}
				if(flag){
					validType = validType.push('numberFmt[18,'+ precision +']');
				}
			}else{
				if(validType.indexOf('numberFmt') == -1){
					validType = [validType].push('numberFmt[18,'+ precision +']');
				}
			}
	...
	$.fn.numberbox = function(_41c, _41d) {
		...
		return this.each(function() {
			...
			if (_41f) {
				...
				//绑定一个币种改变事件
				if(typeof _41f.options.precision  == "string"){
					var _this = this;
					//$('#' + _41f.options.precision).unbind('change.gjjsNumberbox').bind('change.gjjsNumberbox',function(){
					$('#' + _41f.options.precision).bind('change.EBPfixed',function(){
						$(_this).numberbox('setValueNotEvent',$(_this).numberbox('getValue'));
					});
				}
				if($(this).hasClass("P")){//初始化P 样式
					$(this).attr("readonly","readonly");
	...
	$.fn.numberbox.methods = {
		...
		setValueNotEvent : function(jq, _420) {
			return jq.each(function() {
				_40eNotEvent(this, _420);
			});
		},
		getValue : function(jq) {
			return _40f(jq[0]);
	...
	$.fn.numberbox.parseOptions = function(_422) {
		var t = $(_422);
		var options =  $.extend({}, $.fn.validatebox.parseOptions(_422), $.parser
			...
			.extend(
					...
					$.fn.validatebox.defaults,
					{
						...
						parser : function(s) {
							...
							//var val = parseFloat(s).toFixed(opts.precision); modified by tx
							...
							if($.isFunction(opts.precision)){
								...
								//lsm 2014-04-26 关于复合的时候金额对应的币种栏位没有赋值  所以导致小数位错误
								//没有有对应币种栏位而且币种栏位值为空 则取大字段的值
								var curVal = $('#' +  opts.precision).xcpVal();
								if($('#' + opts.precision).length != 0 && curVal == ""){
									if($xcp.formPubMgr){
										curVal  = $xcp.formPubMgr.getFormLoadData();
										curVal  = curVal != null && curVal[opts.precision] ? curVal[opts.precision] : "";
									}
								}
								precision = $xcp.def.getAmtFmtUtil(curVal);
							}
...
(function($) {
	...
	function init(_429) {
		...
		$(_429).find(".calendar-nextmonth").click(function(e) {
			_42a(_429, 1,e);
		});
		$(_429).find(".calendar-prevmonth").click(function(e) {
			_42a(_429, -1,e);
		});
		$(_429).find(".calendar-nextyear").click(function(e) {
			_42d(_429, 1,e);
		});
		$(_429).find(".calendar-prevyear").click(function(e) {
			_42d(_429, -1,e);
		});
	...
	function _4a9(_4aa, _4ab, _4ac) {
		...
		function _4ad(trs1, trs2) {
			for ( var i = 0; i < trs2.length; i++) {
				...
				if(_4b3 >= 125) _4b3 = 25;//lsm1; 会计分录增加备注后ie下第一行高度变成了125px 暂时处理以后需优化
				tr1.css("height", _4b3);
				tr2.css("height", _4b3);
			}
		}
	...
	function _5b0(_5b1, _5b2) {
		...
		function _5b4() {
			var _5b5 = opts.loader.call(_5b1, _5b3, function(data) {
				...
				//增加outEntity modified tx
				if(data != null && data.success =='1'){
					if($xcp.dispAjaxError)$xcp.dispAjaxError($xcp.cloneObject(data));
					data = [];
				}else{
					data = data.outEntity || [];
				}
	...
	var _62e = {
		render : function(_62f, _630, _631) {
			...
			for ( var i = 0; i < rows.length; i++) {
				...
				_634.push("<tr id=\"" + _637 + "\" lsm0='lsm' datagrid-row-index=\"" + i
						+ "\" " + cls + " " + _636 + ">");
			...
			$(_630).html(_634.join("")); // gy 2013-11-22
		...
		renderFooter : function(_638, _639, _63a) {
			...
			$(_639).html(_63c.join("")); // gy 2013-11-22
...
(function($) {
	...
	;
	function init(_78e,opts) {
		
		if (isNaN(opts.width)) {
			  opts.width = $(_78e).hasClass('curSign') || $(_78e).hasClass('narrow')? 50 : 200;
			  //opts.width = $(_78e).outerWidth();
		}
		
		var w = opts.width,h = opts.height,lh = 24;//$(_78e).height() -1;
		
		$(_78e).addClass("combo-f").attr("componentName",opts.componentName || "").hide();
		
		var cls = $(_78e).attr("class");
		var ret = cls.match(new RegExp("\\s*[M|P|O]$|\s*[M|P|O]\\s+"));
		ret = ret != null ? ret[0] : "";
		if($(_78e).hasClass("optional"))ret +=  " optional ";
		
	
	    var span = $("<span class=\"combo\" style='width:" + w + "px;height:" + h + "px'></span>").insertAfter(_78e);
		var _78f = $("<input type=\"text\" class=\"combo-text " + ret + " \" style='width:" + (w-22)+ "px;height:" + h + "px'>").appendTo(span);
		$("<span><span class=\"combo-arrow  \" style='height:" + lh + "px;width:18px;lineHeight:" + lh +"px'></span></span>").appendTo(span);
		...
		if(opts.comboxInitialValue != undefined){
			span.find("input.combo-value").val(opts.comboxInitialValue);
			delete opts.comboxInitialValue;
		}
		
		if(opts.comboxInitialText != undefined){
			span.find("input.combo-text").val(opts.comboxInitialText);
			//delete opts.comboxInitialText;
		}
	...
	function createComboPanel(_78e){
	...
	function _79c(_79d) {
		...
		//if (!opts.disabled && !opts.readonly) {
			_7a1.bind("mousedown.combo", function(e) {
				var _79f = _79e.panel;
			    if(_79f == null){
                	_79f = createComboPanel(_79d);
                }
				$("div.combo-panel").not(_79f).panel("close");
				e.stopPropagation();
			}).bind("keydown.combo", function(e) {
				if($(this).attr("readonly") == "readonly")return;//复核不可编辑 
				var _79f = _79e.panel;
			    if(_79f == null){
                	_79f = createComboPanel(_79d);
                }
				if(opts.xcpResized  !== true) {
					_79f.panel("resize", {
						width : (opts.panelWidth ? opts.panelWidth : _7a0.outerWidth()),
						height : opts.panelHeight
					});
					opts.xcpResized = true;
				}
				switch (e.keyCode) {
				case 38:
	...
	function _7a3(_7a4) {
		...
		if ($.fn.window) {
			if($(_7a4).attr("mainEasyui") == "true"){
				_7a6.panel("panel").css("z-index",10001);
			}else{
		...
		if (_7a6.panel("options").closed) {
			/*if(opts.data != null && opts.data.length > 15){
				opts.panelHeight = '200px';
			}else{
				opts.panelHeight = 'auto';
			}
			_7a6.panel("panel").css("height", opts.panelHeight);*/
			_7a6.panel("open");
			opts.onShowPanel.call(_7a4);
			
			//lsyt add _7a6.panel("move", {
			_7a6.panel("move", {
				left : _7a5.offset().left,
				top : _7a7()
			});
		...
		function _7a7() {
			...
			//lsyt  _7a6.parent() --> _7a6.parent()._outerHeight()
			if (top + 60 + _7a6.parent()._outerHeight() > $(window)._outerHeight()
					+ $(document).scrollTop()) {
				top = _7a5.offset().top - _7a6.parent()._outerHeight();
			}
	...
	function _7ac(_7ad, doit) {
		...
		//gy 统一对COMBOBOX的文件域作校验处理 2013-12-12
		var combobox = $.data(_7ad, "combobox"),id = $(_7ad).attr('id');
		//if(id && combobox != null && !_7ae.attr('comboboxid')){ lsm4
		if(id && combobox != null ){
			_7ae.attr('comboboxid',id);
			var validType = opts.validType;
			if(!validType){
				validType = 'comboboxTextCheck';
			}else{
				validType = $.isArray(validType) ? validType : [validType] ; 
				if ($.inArray('comboboxTextCheck',validType) < 0){
					validType.push('comboboxTextCheck');
				}
			}
			opts.validType = validType;
		}	
	...
	;
	function _7c3(_7c4, _7c5,_notEvent) {
		...
		if(opts.multiple){
			...
			_7c7.find("input.combo-value").val(_7c5[0]);
			$(_7c4).val(_7c5[0]);//ls 2014-11-27  
		}
		...
		if(_notEvent !== false  ){
			if (aa.length != _7c5.length || _7c5.length != _7c6.length) {
				if (opts.multiple) {
					...
					$(_7c4).trigger('change');  // modified  lsm
				} else {
					opts.onChange.call(_7c4, _7c5[0], _7c6[0]);
					$(_7c4).trigger('change');  // modified 
				}
			}
		}		
	}
	...
	;
	function _7cc(_7cd, _7ce,_notEvent) {
		_7c3(_7cd, [ _7ce ],_notEvent);
	}
	;
	function _7cf(_7d0,_notEvent) {
		...
		if (opts.multiple) {
			if (opts.value) {
				if (typeof opts.value == "object") {
					_7c3(_7d0, opts.value,_notEvent);
				} else {
					_7cc(_7d0, opts.value,_notEvent);
				}
			} else {
				_7c3(_7d0, [],_notEvent);
			}
	...
	$.fn.combo = function(_7d1, _7d2) {
		...
		return this.each(function() {
			...
			if (_7d3) {
				...
				var opt = $.extend({}, $.fn.combo.defaults, $.fn.combo.parseOptions(this), _7d1);
				if($(this).hasClass("P")){//初始化P 样式
					opt.readonly = true;
				};
				
				var r = init(this,opt); // lsm combox 加opt 初始化加width
				...
				if(opt.componentName != 'combobox' || opt.multiple){
					_7cf(this,false);
				}
			}
	...
	$.fn.combo.methods = {
		...
		createPanel : function(jq){
			return jq.each(function() {
				createComboPanel(this);
			});
		},
		...
		setValuesNotEvent : function(jq, _7d6) {
			return jq.each(function() {
				_7c3(this, _7d6,false);
			});
		},
		getValue : function(jq) {
			return _7c9(jq[0]);
		...
		setValueNotEvent : function(jq, _7d7) {
			return jq.each(function() {
				_7cc(this, _7d7,false);
			});
		}
	};
...
(function($) {
	...
	function _7e7(_7ee, _7ef, _7f0,_notEvent) {
		...
		// lsm11 上下两个if 互换位置 
		if(_notEvent === false){
			$(_7ee).combo("setValuesNotEvent", vv); 
		}else{
			$(_7ee).combo("setValues", vv);
		}
	}
	;
	function _7f2(_7f3, data, _7f4,noResetValue) {
		var opts = $.data(_7f3, "combobox").options;
		var _7f5 = $(_7f3).combo("panel");
		
		if(_7f5 == null){
			$(_7f3).combo("createPanel");
			_7f5 =  $(_7f3).combo("panel");
		}
		//if(opts.panelContentInited !== true){
		$(_7f3).combo("panel").unbind().bind(
				"mouseover",
				function(e) {
		...
		for ( var i = 0; i < data.length; i++) {
			...
			var item = s;		
			if (opts.formatter) {
				item = opts.formatter.call(_7f3, data[i]);
			}
			
			var str = "";
			if(item == ""){//lsyt
				str = " style='height:16px;' ";
			}
			//skf 下拉框奇偶行变色 
			if(i%2==0){
				html += "<div class=\"combobox-item combobox_odd\" value='" + v + "' " + str + ">" + item + "</div>";
				//item = $("<div class=\"combobox-item combobox_odd\" ></div>").appendTo(_7f5);
			}else{
				html += "<div class=\"combobox-item combobox_even\" value='" + v + "' " + str + ">" + item + "</div>";
				//item = $("<div class=\"combobox-item combobox_even\" ></div>").appendTo(_7f5);
			}
			//item.attr("value", v);
		...
		if(noResetValue === false){
			if (opts.multiple) {
				_7e7(_7f3, _7f6, _7f4,false);
			} else { 
				if (_7f6.length) {
					_7e7(_7f3, [ _7f6[_7f6.length - 1] ], _7f4,false);
				} else {					 
					_7e7(_7f3, [], _7f4,false);
				}
			}
		}
		if($(_7f3).xcpVal() != "" || $(_7f3).val() != ""){
			$(_7f3).combobox("select", $(_7f3).xcpVal() == "" ? $(_7f3).val() : $(_7f3).xcpVal());
		}
	...
	function _7fb(_7fc, q) {
		...
		if (opts.mode == "remote") {
			...
			if( q == ''){
				...
				var itemObj = _7fd.find("div.combobox-item").hide();//combobox 模糊查询优化
				...
				$.each(itemObj,function(i,o){
					var text = $(o).html();
					var val = $(o).attr("value");
					if (text.toUpperCase().indexOf(q.toUpperCase()) != -1 ) {
						$(o).show();
						if (text == q) {
							_7e7(_7fc, [ val ], true);
							$(o).addClass("combobox-item-selected");
						}
					}
				});
	...
	function _802(_803) {
		...
		$(_803).combo($.extend({}, opts, {
			onShowPanel : function() {			
				if(opts.panelContentInited == false){
					_7f2(_803, opts.data || [],false,false);				
				}
	...
	function _asyncLoadDataProcess(_801,data,value) {						
		  data = data || [];
		  var target = $.data(_801, "combobox");
		  var opt = target.options;
		  target.data = opt.data = data;		
		 
		  if(value != undefined && value != ""){			
				var item = data.length > 0 ? _7db(data,opt.valueField,value) : null;
				var text = item == null ? "" : item[opt.textField];			
				$(_801).combo("textbox").val(text);
				$.data(_801, "combo").previousValue = text;			
		  }

		  delete target.options.loadDataKey;
		  delete target.options.isAsyncLoadData;			
	};

	$.fn.combobox = function(_805, _806) {
		
		if (typeof _805 == "string") {
			var combobox = $.data(this, "combobox");
			/*if(combobox == null){
				$(this).combobox();
			}*/
			if(!$(this).attr('componentName')){
			    $(this).combobox();
		    }
		...
		return this.each(function() {
			...
			if (_808) {
				...
				_802(this);
				if (_805.data != null && _808.options.data) {
					_808.options.panelContentInited = true;
					_7f2(this, _808.options.data);
					/*//动态给combobox赋值的时候 选择对应的text值  如果存在效率问题可以换种解决思路
					if($(this).xcpVal() != ""){
						$(this).combobox("select", $(this).xcpVal());
					}*/
				}	
			} else {
				var _this = this;				
				_808 = $.data(this, "combobox", {
					options : $.extend({componentName:'combobox'}, $.fn.combobox.defaults,$.fn.combobox.parseOptions(this), _805)
				});				
				
				var opt = _808.options;				
				opt.panelContentInited = false;
				if(typeof opt.data == 'string' && opt.data != ''){
					 opt.isAsyncLoadData = true; 
					 opt.loadDataKey = opt.data;
					 opt.data = [];
				}else{				
					opt.data = opt.data || [];
					if(opt.data.length == 0){
						opt.data = $.fn.combobox.parseData(this);
					}
				}
				
				if(opt.data && opt.data.length > 15){
					opt.panelHeight = 200;
				}
				
				/*gy 赋combobox 初始值 start */
				var value = $(this).val() ;
				if(value == undefined || value == "")
					value = opt.value;
				if(!opt.multiple &&  value != undefined && value != "" ){
					var item = opt.data.length > 0 ? _7db(opt.data,opt.valueField,value) : null;
					var text = item == null ? "" : item[opt.textField];
					opt.comboxInitialValue = value;
					opt.comboxInitialText  = text == "" && opt.data.length == 0 ? value : text;
				}
				/*gy 赋combobox 初始值 end */
			
				_802(this);
			
				$.data(this, "combobox").data = opt.data;
				
				if(value != undefined && value != "" ){
					if(opt.multiple){
						_7e7(this,[value], false,false);
					}else if(opt.comboxInitialText != undefined){
						 $.data(this, "combo").previousValue = opt.comboxInitialText;
					}					
				}

				if(opt.isAsyncLoadData == true){					
					$xcp.getAsyncConstant(
						function(data){
							_asyncLoadDataProcess(_this,data,value);
					},opt.loadDataKey);					
				}
			}
		});
	...
	$.fn.combobox.methods = {
		...
		setValueNotEvent : function(jq, _80b) {
			return jq.each(function() {
				_7e7(this, [ _80b ],false,false);
			});
		},
		clear : function(jq) {
			return jq.each(function() {
		...
		readonly : function(jq,mode){ //20141223 tx
			return jq.each(function(){
				$(this).combobox("options").readonly = true;
...
(function($) {
	...
	;
	function _820(_821, _822,_blean) {
		...
		if(_blean == false){
			$(_821).combo("setValuesNotEvent", vv)
					.combo("setText", ss.join(opts.separator));
		}else{
	...
	$.fn.combotree = function(_823, _824) {
		...
		return this.each(function() {
			...
			if (_826) {
				...
				$.data(this, "combotree", {
					options : $.extend({componentName:'combotree'}, $.fn.combotree.defaults,
	...
	$.fn.combotree.methods = {
		...
		setValuesNotEvent : function(jq, _828) {
			return jq.each(function() {
				_820(this, _828,false);
			});
		},
		...
		setValueNotEvent : function(jq, _829) {
			return jq.each(function() {
				_820(this, [ _829 ],false);
			});
		},
		clear : function(jq) {
			return jq.each(function() {
...
(function($) {
	function _852(_853) {
		...
		$(_853).combo($.extend({}, opts, {
			onShowPanel : function() {
				if (!_854.calendar) {
				   ...
				   //ls add 2014-11-27
				   if(_853.value != ""){
					   _854.calendar.calendar("moveTo", opts.parser(luangeForDate(_dateTransform(_853.value))));
				   }
				}
		...
		//增加必输项等样式的显示问题  gy append by 2013-12-18
		var cls = $(_853).attr('class');
		if(cls != ''){
			cls = " " + cls.replace('easyui-datebox','');
		}
		
		$(_853).combo("textbox").parent().addClass("datebox");
		$(_853).combo("textbox").addClass("datebox-text" + cls);
		
		$(_853).combo("textbox").hover(function() {
			$(this).tooltip({
				position:'right',
				deltaX : opts.deltaX,
				content: "Date Format : " + $.fn.datebox.defaults.dateFormat,
				style:{
					color : "#000",
					borderColor : "#CC9933",
					backgroundColor : "#FFFFCC"
		...
		//日期空间 onchange事件 处理 不要输入时触发 移开以后触发
		//修改combo 的默认添加事件自己添加操作和jquery的change事件  modified
		 
		$(_853).combo("textbox").unbind('keydown.combo').unbind(
				'change.combo').bind('change.combo', function() {
					//日期格式修改 xhb
					var dateVal = $(this).val();
					var dateR = new RegExp("\\d{8}");
					if(dateR.test(dateVal)){
						dateVal = dateVal.substring(0,4) + "-" + dateVal.substring(4,6) + "-" + dateVal.substring(6,8);
					}
					$(_853).datebox("setValue", dateVal);
//			$(_853).datebox("setValue", $(this).val());
			//$(_853).trigger('change');
		});
		...
		function _855() {
			...
			//_85b(_853, opts.value);   ls 2013 11-28 
			var _858 = $("<div class=\"datebox-button\"></div>").appendTo(_856);
			$("<a href=\"javascript:void(0)\" class=\"datebox-current\">"+opts.currentText+"</a>").appendTo(_858);
		
			//$("<a href=\"javascript:void(0)\" class=\"datebox-close\">"+opts.closeText+"</a>").appendTo(_858);
	...
	;
	// ls- 日期转换函数 统一返回YYYY-MM-DD 支持mm/dd/yyyy yyyy/mm/dd yyyy-mm-dd格式 modified
	function _dateTransform(_date) {
		var new_date = "";
		if (_date) {
			_date = _date.replace(" 00:00:00",""); 
			var str = _date.split("/");
			if (str.length == 3) {
				new_date = (str[0].length == 4 ? str[0] + "-"
							+ (str[1] < 10 && str[1].length == 1  ? ('0' + str[1]) : str[1]) + "-"
							+ (str[2] < 10 && str[2].length == 1  ? ('0' + str[2]) : str[2]) : str[2]
							+ "-" + (str[0] < 10 && str[0].length == 1  ? ('0' + str[0]) : str[0]) + "-"
							+ (str[1] < 10 && str[1].length == 1  ? ('0' + str[1]) : str[1]));
			}
			if (_date.split("-").length == 3) {
				str = _date.split("-");
				//2014-10-23 modified by tx ,desc "1-1-1"这种格式校验
				if(str[0].length < 4){
					new_date = "";
					return new_date;
				}
				new_date = str[0]
						+ "-"
						+ (str[1] < 10 && str[1].length == 1 ? ('0' + str[1])
								: str[1])
						+ "-"
						+ (str[2] < 10 && str[2].length == 1 ? ('0' + str[2])
								: str[2]);
			}
		}
		return new_date;
	...
	//根据环境设置显示出来的值
	function luangeForDate(_date){
		if (_date) {
			_date = _date.replace(" 00:00:00",""); // modified
			var str = null;
			if(EAP_LANGUAGE){
				switch (EAP_LANGUAGE) {
				case "en_US":
	...
	function _85b(_85f, _860,_notEvent) {
		var _861 = $.data(_85f, "datebox");
		var opts = _861.options;
		
		var _980 = _dateTransform(_860); // ls- 保证js获取到的val值都是正确格式 但不验证日期是否有效  modified
		_860 = luangeForDate(_980);  // 显示符合当前语言环境的日期格式
		
		if( _notEvent !== false){
			$(_85f).combo("setValue", _980);
			$(_85f).combo("setText", _860);
			
		}else {
			$(_85f).combo("setText", _860);
			$(_85f).combo("setValueNotEvent", _980);
		}
		if(_861.calendar){
	...
	$.fn.datebox = function(_862, _863) {
		...
		return this.each(function() {
			...
			if (_865) {
				...
				$.data(this, "datebox", {
					options : $.extend({componentName:'datebox'}, $.fn.datebox.defaults, $.fn.datebox
							.parseOptions(this), _862)
				});
			}
			if($xcp.formPubMgr && $xcp.formPubMgr.setDateValidType)$xcp.formPubMgr.setDateValidType(this);
			_852(this);
			
			var value = $(this).val();
			if(value != ""){
				_85b(this, value,false);
			}
		});
	};
	$.fn.datebox.methods = {
		...
		setValueNotEvent : function(jq, _867) {
			return jq.each(function() {
				_85b(this, _867,false);
			});
		},
		reset : function(jq) {
			return jq.each(function() {
		...
		getDateFormat : function(){
			return $.fn.datebox.defaults.dateFormat;//lsm
		}
	};
...
(function($) {
	function _869(_86a) {
		...
		$(_86a).next("span").addClass("datetimeSpan");//lsm datetimebox
